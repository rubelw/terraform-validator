...
class TestCloudDistribution(unittest.TestCase):
    ...
    def test_cloudformation(self):
        ...
        expected_result = [
            {
                "failure_count": "0",
                "filename": "/tf/cloudfront_distribution/cloudfront_distribution_without_logging.tf",
                "file_results": [
                    {
                        "id": "W1",
        ...
        template_name = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/terraform_validator/test_templates/tf/cloudfront_distribution/cloudfront_distribution_without_logging.tf'
        debug = True
        ...
        self.maxDiff = None
        self.assertEqual(json.loads(expected_result.replace('\'','"')), json.loads(real_results))
