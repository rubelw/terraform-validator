...
class TestLambdaPermission(unittest.TestCase):
    ...
    def test_lambda_permission_with_some_out_of_ord_items(self):
      ...
      expected_result = [
            {
                'failure_count': '2',
                'filename': '/tf/lambda_permission/lambda_with_wildcard_principal_and_non_invoke_function_permission.tf',
                'file_results': [
                    {
                        'id': 'F13',
      ...
      template_name = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/terraform_validator/test_templates/tf/lambda_permission/lambda_with_wildcard_principal_and_non_invoke_function_permission.tf'
      debug = True
