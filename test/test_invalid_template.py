...
class TestEc2Instance(unittest.TestCase):
    ...
    def test_ec2_invalid_template(self):
      ...
      expected_result =  [
            {
                'failure_count': '0',
                'filename': '/tf/ec2_instance/invalid_template.tf',
                'file_results': [
                    {
                        'id': 'W1',
      ...
      template_name = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/terraform_validator/test_templates/tf/ec2_instance/invalid_template.tf'
      debug = True
