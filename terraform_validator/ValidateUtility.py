...
import tempfile
import shutil
from terraform_model.parser import CfnParser
from terraform_validator.CustomRuleLoader import CustomRuleLoader
from terraform_validator.ProfileLoader import ProfileLoader
from terraform_validator.TemplateDiscovery import TemplateDiscovery
from terraform_validator.Violation import Violation
from terraform_validator.result_views.JsonResults import JsonResults
from terraform_validator.result_views.SimpleStdoutResults import SimpleStdoutResults
...
class ValidateUtility:
    ...
    def validate(self):
        ...
        if self.debug:
            ...
            print('\n\n############################################')
            print('audit_audit_aggregate_across_files_and_render_results '+lineno())
            print('################################################\n\n')
    ...
    def merge_dicts(self, x, y):
        ...
        if self.debug:
            print('x: '+str(x)+lineno())
            print('y:'+str(y)+lineno())

        z = x.copy()  # start with x's keys and values
        z.update(y)  # modifies z with y's keys and values & returns None

        if self.debug:
            print('z: '+str(z)+lineno())
    ...
    def audit_aggregate_across_files(self,input_path=None, parameter_values_path=None, template_pattern=None):
        ...
        for template in templates:
            ...
            if json_matchObj:
                ...
                with open(template, 'r') as myfile:
                    ...
                    try:
                        file_data = file_data + str(json.loads(json_acceptable_string))
                    except Exception as e:
                ...
                #tf.add_multi_constructor('', default_constructor, Loader=yaml.SafeLoader)
                #tf.add_representer(GenericScalar, GenericScalar.to_yaml, Dumper=yaml.SafeDumper)

                with open(template, 'r') as myfile:

                    if self.debug:
                        print("\n#######################################")
                        print('template read: '+str(template)+lineno())
                        print("######################################\n")
                    try:
                        ...
                        print("\n##############################")
                        print("Invalid tf file - " + str(template) + ' - skipping file')
                        print('error: '+str(e))
                        print("################################\n")

                    #file_results = self.audit(cloudformation_string=file_data,
                ...
                if self.debug:
                    print('not sure what this is: '+lineno())
                #try:
                #    json_stuff = open(template)
                #    my_template = json.load(json_stuff)

                #    if my_template and ('resources' in my_template or 'variable' in my_template):
                #        if self.debug:
                #            print('might be terraform '+lineno())
        ...
        #    file_data= json.loads(json_acceptable_string)
    ...
    def audit(self, cloudformation_string, parameter_values_string=None):
        ...
        # We should validate the schema, just now sure how to do that right now, and don't
        ...
        try:
            ...
            if self.debug:
                print("\n#########################")
                print("Starting to parse the template "+lineno())
                print('trying to parse cloudformation string: '+str(cloudformation_string)+lineno())
                print("###########################\n")
