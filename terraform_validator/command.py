...
from terraform_validator import ValidateUtility
from terraform_validator.RuleDumper import RuleDumper
...
@click.group()
@click.version_option(version='0.6.41')
def cli():
    pass
...
def myversion():
    ...
    '''
    print('Version: ' + str(terraform_validator.__version__))
...
def get_current_pip_version(debug=False):
    ...
    try:
        command = 'pip search terraform-validator | grep terraform-validator | cut -d\' \' -f 2 |  sed -e "s/^(//" -e "s/)//"'
        if debug:
        ...
        print('Error trying to determine the current terraform-validator version')
        return 0
...
def check_for_updates(debug=False):
    ...
    try:
        ...
        if sys.version_info[0] < 3:
            ...
            current_local_version = str(terraform_validator.__version__).rstrip('\n').strip().decode("utf-8")
        ...
        if current_pip_version == 0:
            ...
            print('#########################################################################################')
            print('There is a more current version of terraform-validator. You should update ')
            print('terraform-validator with pip install -U terraform-validator')
            print('#########################################################################################')
            sys.exit(1)
        else:
            if debug:
                print('terraform-validator is the most current version')
    except Exception as e:
        print('Error trying to determine the current terraform-validator version')
